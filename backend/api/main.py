...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request, call_next):
        ...
        # Content-Security-Policy is intentionally permissive for local dev;
        # tighten it (e.g., remove 'unsafe-inline', 'unsafe-eval') when a nonce
        # system is in place for the frontend.
...
def _cors_origins() -> list[str]:
    """Return allowed CORS origins. In production only explicit origins are
    permitted; development falls back to common local ports if none configured.
...
async def productivity_developers(
    ...
    """
    if sprint_done_filter:
        jira_sql += " " + sprint_done_filter
    if jira_time_filter:
        jira_sql += " " + jira_time_filter
    jira_sql += " GROUP BY meta->>'assignee_login'"
    jira_rows = db.execute(text(jira_sql), jira_params).mappings().all()
